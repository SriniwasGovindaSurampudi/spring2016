...
%% Laplacian linear model
    ...
    [corCoef,Pval.pval{jj}] = corrcoef(vectorize_matrix(Mtx.H{jj} .* inds),vectorize_matrix(corr_graphs .* inds));
...
for ii=1:nt
    ...
    [Err,pval{ii}] = corrcoef(vectorize_matrix(Hhat.*inds), vectorize_matrix(corr_graphs.*inds));% corrcoef((Hhat.*inds), (corr_graphs.*inds));  % corrcoef(Hhat(inds),corr_graphs(inds));
    corCoef(ii) = Err(1,2);
end
